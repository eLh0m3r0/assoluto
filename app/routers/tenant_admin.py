...
async def sla_dashboard(
    ...
    try:
        ...
        # Log full traceback AND echo to the response so production debugging
        # is possible without SSH — only for logged-in tenant staff. Remove
        # once the root cause is fixed.
        tb = traceback.format_exc()
        log.exception("sla_dashboard failed: %s", exc)
        return HTMLResponse(
            f"<pre style='padding:1rem;font:12px monospace;white-space:pre-wrap'>"
            f"SLA DEBUG — error: {exc}\n\n{tb}</pre>",
            status_code=500,
        )

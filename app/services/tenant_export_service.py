...
_TABLES: list[tuple[str, Any, list[str]]] = [
    (
        ...
        Order,
        [
            "id", "number", "title", "status", "customer_id", "currency",
            "quoted_total", "notes", "requested_delivery_at", "promised_delivery_at",
            "submitted_at", "delivered_at", "closed_at", "created_at", "updated_at",
        ],
    ),
    (
        "order_items",
        OrderItem,
        ["id", "order_id", "product_id", "description", "quantity", "unit",
         "unit_price", "line_total", "position", "created_at"],
    ),
    (
        "order_comments",
        OrderComment,
        ["id", "order_id", "body", "is_internal", "author_user_id",
         "author_contact_id", "created_at"],
    ),
    (
        "order_status_history",
        OrderStatusHistory,
        ["id", "order_id", "from_status", "to_status", "note",
         "changed_by_user_id", "changed_by_contact_id", "created_at"],
    ),
    (
        "customers",
        Customer,
    ...
    (
        "customer_contacts",
        CustomerContact,
        ["id", "customer_id", "email", "full_name", "is_active",
         "invited_at", "accepted_at", "last_login_at", "created_at"],
    ),
    (
        "products",
        Product,
        ["id", "sku", "name", "unit", "default_price", "customer_id",
    ...
    (
        "assets",
        Asset,
        ["id", "customer_id", "name", "code", "description", "unit",
         "current_quantity", "location", "is_active", "created_at"],
    ),
    (
        "asset_movements",
        AssetMovement,
        ["id", "asset_id", "type", "quantity", "note", "occurred_at",
         "reference_order_id", "created_by_user_id", "created_at"],
    ),
    (
        "order_attachments",
        OrderAttachment,
        ["id", "order_id", "order_item_id", "kind", "filename", "content_type",

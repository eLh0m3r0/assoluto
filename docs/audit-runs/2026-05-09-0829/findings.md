...
- **Auto-fixable**: no
- **status**: open (manual — needs URL architecture decision: subpath locales vs query-string locales)
...
- **status**: open (manual — operator must set STRIPE_PRICE_STARTER / STRIPE_PRICE_PRO)
...
- **status**: manual (deferred — needs proper test design)
...
- **Auto-fixable**: no
- **status**: open (manual — editorial decision: dual-currency display vs CZK-only)
...
- **status**: manual (deferred — needs product decision: self-service vs org-admin override)
...
- **Auto-fixable**: no
- **status**: open (manual — extend handler set per product judgement)
...
- **Auto-fixable**: no
- **status**: open (manual — partially closed by F-BE-007/008 fixes; the comment-author render assertion (075a957) remains)
